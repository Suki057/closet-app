  ...
  var store = {
    ...
    init: function () {
      return Promise.all([db.all('items'), db.all('looks')]).then(function (res) {
        ...
        // 旧单品的"原图即缩略图"不再启动时批量解码（会抢占手机 CPU 导致卡顿），
        // 改为卡片进入视口 + 浏览器空闲时由渲染层调用 CL.store.requestThumb 懒降级。
    ...
    /* 懒迁移：旧单品"原图即缩略图"的全尺寸图，按"进入视口 + 浏览器空闲"逐个降级为小图。
       不再启动时全局批量解码（那会抢占手机 CPU 导致整体卡顿），而是谁被看到、且浏览器空闲时才处理，
       处理完就地替换卡片图并存库，不触发整页重绘。CL.makeThumb 在 app.js 顶层挂载。 */
    _thumbQ: [],
    _thumbBusy: false,
    _pumpThumbs: function () {
      var self = this;
      if (self._thumbBusy) return;
      var id = self._thumbQ.shift();
      if (id === undefined) return;
      var it = store.getItem(id);
      if (!it || it.deletedAt || it.thumbV === 1) { self._pumpThumbs(); return; }
      var src = it.imgFull || it.img;
      if (!src) { it.thumbV = 1; self._pumpThumbs(); return; }
      self._thumbBusy = true;
      CL.makeThumb(src, 480, 0.72).then(function (thumb) {
        if (thumb) {
          it.img = thumb; it.thumbUrl = thumb; it.thumbV = 1;
          var im = document.querySelector('.card[data-id="' + it.id + '"] img');
          if (im) im.src = thumb;
          self.saveThumb(it.id, thumb);
        } else { it.thumbV = 1; }
        self._thumbBusy = false;
        if (self._thumbQ.length) {
          if (window.requestIdleCallback) window.requestIdleCallback(function () { self._pumpThumbs(); }, { timeout: 1000 });
          else setTimeout(function () { self._pumpThumbs(); }, 140);
        }
      });
    },
    /* 渲染层调用：某张卡片进入视口时登记，空闲时再降级，避免一次性解码全部大图卡 UI */
    requestThumb: function (id) {
      if (this._thumbQ.indexOf(id) !== -1) return;
      this._thumbQ.push(id);
      var self = this;
      if (window.requestIdleCallback) window.requestIdleCallback(function () { self._pumpThumbs(); }, { timeout: 1000 });
      else setTimeout(function () { self._pumpThumbs(); }, 140);
    },
